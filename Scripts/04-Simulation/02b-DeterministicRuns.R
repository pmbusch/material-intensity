...
SUPER_BY_SUBUSE <- c(
  "residential" = "buildings", "non_residential" = "buildings",
  "roads" = "civil", "civil_engineering" = "civil",
  "machinery_group" = "machinery", "vehicles_group" = "machinery",
  "durables" = "sl_products", "packaging" = "sl_products"
)
...
intensity_bounds_biomass <- intensity_raw |>
  ...
  dplyr::mutate(mat_key = dplyr::case_when(
  ...
  dplyr::select(mat_key, int_min = min, int_max = max, mid_value)
...
intensity_bounds_nonmet <- intensity_raw |>
  dplyr::filter(stringr::str_detect(tolower(Category), "non.?metallic")) |>
  dplyr::mutate(mat_key = dplyr::case_when(
    stringr::str_detect(tolower(Detail), "build")  ~ "buildings",
    stringr::str_detect(tolower(Detail), "civil")  ~ "civil",
    stringr::str_detect(tolower(Detail), "short")  ~ "sl_products",
...
for (col in u_col_names) mc_input_matrix[[col]] <- 0.5
...
unep_dmc <- unep_dmc |>
  ...
  mutate(material_category = case_when(
    ...
    Material_group == "Fossil fuels" &
      !(material_category %in% unname(FOSSIL_LABEL)) ~ "Other fossil fuels",
    T ~ material_category
  )) %>%
...
stock_2024_raw <- readr::read_csv("Parameters/stock_2024_total.csv", show_col_types = FALSE) |>
...
stock_2024_sub <- stock_2024_raw |>
  ...
  lapply(function(md) {
    split(md, ~region) |>
      lapply(function(rd) setNames(rd$stock_Mt, rd$sub_use))
  })
...
build_age_lookup <- function(material_label) {
  ...
  for (rg in unique(df$region)) {
    ...
    for (su in unique(df$sub_use[df$region == rg])) {
      ...
      out[[rg]][[su]] <- list(cohort_years = as.integer(sub$cohort_year),
...
make_mat <- function(df, val_col) {
  ...
  for (s in ssp_labels_all) {
    ...
    m <- matrix(NA_real_, nrow = length(regions_vec), ncol = N_YR,
...
build_endpoints <- function(bounds, intensity_bounds, mc, lambda_col, fixed_keys = character(0)) {
  ...
  if (length(mat_keys) == 0) return(NULL)
  ...
  G_df <- intensity_bounds |>
    ...
    dplyr::select(mat_key, G = mid_value) |>
  ...
  sampled <- bounds |>
    ...
    mutate(endpoint = G * (int_2024 / pmax(int_2024_wavg, 1e-12))^lambda,
...
ep_biomass <- build_endpoints(biomass_bounds, intensity_bounds_biomass, mc_input_matrix,
                               "gap_persistence_biomass",
                               fixed_keys = "other_biomass") |>
  mutate(material_group = "biomass")

ep_fossil <- build_endpoints(fossil_bounds, intensity_bounds_fossil, mc_input_matrix,
                              "gap_persistence_fossilfuels",
                              fixed_keys = "other_fossil") |>
  mutate(material_group = "fossil_fuels")

ep_metal <- build_endpoints(metal_bounds, intensity_bounds_metal, mc_input_matrix,
                             "gap_persistence_metal_construction",
                             fixed_keys = c("sl_products", "machinery")) |>
  mutate(material_group = "metal_ores")

ep_nonmet <- build_endpoints(nonmet_bounds, intensity_bounds_nonmet, mc_input_matrix,
                              "gap_persistence_metal_construction",
                              fixed_keys = c("sl_products", "machinery")) |>
...
recyc_conv_yr <- mc_input_matrix |>
  ...
  mutate(recyc_convergence_yr = as.integer(round(
...
recycling_traj_fe    <- make_recycling_traj(recycling_now_fe,    "recycling_Fe_global",    RECYCLING_RATE_FE_MIN,    RECYCLING_RATE_FE_MAX)
recycling_traj_nonfe <- make_recycling_traj(recycling_now_nonfe, "recycling_NonFe_global", RECYCLING_RATE_NONFE_MIN, RECYCLING_RATE_NONFE_MAX)
...
downcycling_long <- downcycling_now |>
  tidyr::pivot_longer(
    ...
    names_to = "end_use", names_pattern = "downcycling_(.+)_now", values_to = "rate_now")
...
downcycling_G <- mc_input_matrix |>
  ...
  tidyr::pivot_longer(-run_id, names_to = "end_use", values_to = "u",
                      names_pattern = "downcycling_(.+)_global") |>
...
lifetime_dr <- mc_input_matrix |>
  ...
  mutate(mean_life = pmax(LIFETIME_MIN, mean_life_min + u_mean * (mean_life_max - mean_life_min)),
...
grade_params <- mc_input_matrix |>
  ...
  mutate(grade_ore_fe    = GRADE_ORE_FE_MIN    + grade_ore_fe_u    * (GRADE_ORE_FE_MAX    - GRADE_ORE_FE_MIN),
...
scalar_params <- mc_input_matrix |>
  dplyr::select(run_id, u2 = sub_factor_recycling_same,
                u2b = sub_factor_recycling_same_civil, u3 = max_secondary_roads,
  ...
  dplyr::select(run_id, sub_factor_recycling_same,
                sub_factor_recycling_same_civil, max_secondary_roads, sub_factor_downcycling_roads)
...
run_one <- function(i) {
  ...
  for (j in seq_len(nrow(bio))) {
    rg <- bio$region[j]; ul <- bio$unep_label[j]
    ...
    if (any(is.na(M_Mt))) next
    out_list[[length(out_list) + 1L]] <- data.frame(
      run_id = i, ssp_label = ssp, region = rg,
      material_group = "biomass", material_key = ul, year = yr_vec,
      total_inflow_Mt = M_Mt, in_use_stock_Mt = NA_real_,
      primary_consumption_Mt = M_Mt, secondary_supply_Mt = 0
    )
  }
  ...
  for (j in seq_len(nrow(crops_rows))) {
    ...
    out_list[[length(out_list) + 1L]] <- data.frame(
      run_id = i, ssp_label = ssp, region = rg,
      material_group = "biomass", material_key = "Crop Residues", year = yr_vec,
      total_inflow_Mt = M_Mt, in_use_stock_Mt = NA_real_,
      primary_consumption_Mt = M_Mt, secondary_supply_Mt = 0
    )
  }
  ...
  for (j in seq_len(nrow(fos))) {
    rg <- fos$region[j]; mk <- fos$mat_key[j]
    if (is.na(fos$M_2024_Mt[j])) next
    mg_ratio <- fos$int_2024[j] + (fos$endpoint[j] - fos$int_2024[j]) * alpha
    mg_index <- mg_ratio / max(abs(fos$int_2024[j]), 1e-12)
    M_Mt <- fos$M_2024_Mt[j] * pop_i[rg, ] * gdppc_i[rg, ] * mg_index
    out_list[[length(out_list) + 1L]] <- data.frame(
      run_id = i, ssp_label = ssp, region = rg,
      material_group = "fossil_fuels", material_key = FOSSIL_LABEL[mk], year = yr_vec,
      total_inflow_Mt = M_Mt, in_use_stock_Mt = NA_real_,
      primary_consumption_Mt = M_Mt, secondary_supply_Mt = 0
    )
  }
  ...
  run_dsm_metal <- function(mat_label, age_lookup, rec_i, grade) {
    ie_g <- ie_i |>
      ...
      mutate(mat_key_super = mat_key)

    sr_rows <- list()
    for (j in seq_len(nrow(ie_g))) {
      rg <- ie_g$region[j]; mk <- ie_g$mat_key_super[j]
      su_list <- SUB_USE_BY_MATKEY[[mk]]
      if (is.null(su_list)) next
      stock_intensity <- ie_g$int_2024[j] + (ie_g$endpoint[j] - ie_g$int_2024[j]) * alpha
      ...
      for (su in su_list) {
        ...
        if (is.null(s2024) || s2024 <= 0) next
        target_stock <- s2024 * pop_i[rg, ] * gdppc_i[rg, ] * stock_intensity_index
        lp_row <- lp_i[lp_i$sub_use == su, ]
        if (nrow(lp_row) == 0) next
        res <- run_forward_dsm_fast(
          cohort_years = age$cohort_years, cohort_stocks_2024 = age$cohort_stocks,
          target_stock = target_stock, mean_life = lp_row$mean_life[1], k = lp_row$weibull_k[1],
          start_year = 2024L, end_year = FORECAST_END
        )
        sr_rows[[length(sr_rows) + 1L]] <- data.frame(
          region = rg, sub_use = su, year = res$year,
          total_stock_Mt = res$total_stock, new_additions_Mt = res$new_additions,
          replacement_Mt = res$replacement, production_Mt = res$production,
          waste_Mt = res$waste, target_stock_Mt = target_stock
        )
      }
    }
    if (length(sr_rows) == 0) return(NULL)
    ...
    recovered <- pmin(sr$waste_Mt * recyc_rate, prod_metal)
    primary_metal_Mt <- prod_metal - recovered
    grade_safe <- max(grade, 1e-6)
    primary_ore_Mt <- primary_metal_Mt / grade_safe
    data.frame(
      run_id = i, ssp_label = ssp, region = sr$region,
      material_group = if (mat_label == "Metal_Fe") "metal_fe" else "metal_nonfe",
      material_key = SUB_USE_LABELS[sr$sub_use], year = sr$year,
      total_inflow_Mt = sr$replacement_Mt + sr$new_additions_Mt,
      in_use_stock_Mt = sr$total_stock_Mt,
      primary_consumption_Mt = primary_ore_Mt,
      secondary_supply_Mt = recovered,
      new_additions_Mt = sr$new_additions_Mt, replacement_Mt = sr$replacement_Mt,
      waste_Mt = sr$waste_Mt, target_stock_Mt = sr$target_stock_Mt
    )
  }
  ...
  if (!is.null(fe_out))    out_list[[length(out_list) + 1L]] <- fe_out
  if (!is.null(nonfe_out)) out_list[[length(out_list) + 1L]] <- nonfe_out
  ...
  for (j in seq_len(nrow(ie_g_nm))) {
    rg <- ie_g_nm$region[j]; mk <- ie_g_nm$mat_key_super[j]
    su_list <- SUB_USE_BY_MATKEY[[mk]]
    if (is.null(su_list)) next
    stock_intensity <- ie_g_nm$int_2024[j] + (ie_g_nm$endpoint[j] - ie_g_nm$int_2024[j]) * alpha
    ...
    for (su in su_list) {
      ...
      nm_rows[[length(nm_rows) + 1L]] <- data.frame(
        region = rg, sub_use = su, super_key = mk, year = res$year,
        total_stock_Mt = res$total_stock, new_additions_Mt = res$new_additions,
        replacement_Mt = res$replacement, production_Mt = res$production,
        waste_Mt = res$waste, target_stock_Mt = target_stock
      )
  ...
  if (length(nm_rows) > 0) {
    ...
    rd <- data.frame(
      region = nm_sr$region, year = nm_sr$year,
    ...
    recovered <- ifelse(
      is_bldg, recovered_same,
      ifelse(is_roads, ifelse(is.na(nm_sr$sec_for_roads), 0, nm_sr$sec_for_roads),
        ifelse(is_civil & !is_roads, recovered_same, 0))
    )
    nm_sr$production_Mt <- pmax(0, nm_sr$production_Mt - recovered)

    out_list[[length(out_list) + 1L]] <- data.frame(
      run_id = i, ssp_label = ssp, region = nm_sr$region,
      material_group = "nonmetallic_minerals",
      material_key = SUB_USE_LABELS[nm_sr$sub_use], year = nm_sr$year,
      ...
      secondary_supply_Mt = recovered,
      new_additions_Mt = nm_sr$new_additions_Mt, replacement_Mt = nm_sr$replacement_Mt,
      waste_Mt = nm_sr$waste_Mt, target_stock_Mt = nm_sr$target_stock_Mt
    )
  }
