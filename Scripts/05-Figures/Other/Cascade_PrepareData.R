## =============================================================================
## Cascade_PrepareData.R
## Builds the Kaya cascade-waterfall data behind Cascade.R: decomposes the
...
## (median of all MC runs; the existing Figure 4 top-10% "decoupling" subset;
## and the Figure 4 CAGR-based "Absolute decoupling" class).
##
...
## Figure 2 - ProjectionMethod.R (SECTION F2).
##
## Output (Parameters/Intermediate/):
##   Cascade.csv - one row per bar per panel (66 rows: material_group x
...
cat("=== Cascade - Prepare Data ===\n\n")

# Constants ---------------------------------------------------------------

# Full raw-category buckets (the "nec./other" rows folded in) -- NOT the
# narrower list Figure 2 - ProjectionMethod.R SECTION F uses for its historical
...
# Same SSP-blend reconstruction as Figure 2 - ProjectionMethod.R SECTION C2,
...
# kind of mismatch Figure 2 - ProjectionMethod.R SECTION F2 already documents),
...
# decoupling" (the rigorous CAGR-based decoupling_class from Figure 4, not the
...
# ended (cumsum/lag idiom, same as Figure 1 - TimeSeries.R:58 / Figure 4 -
...
bars <- kaya |>
  ...
  mutate(
    ...
    # of that SAME material's colour (resolved in Cascade.R) so the
...
write_csv(bars, "Parameters/Intermediate/Cascade.csv")
cat("  Saved: Parameters/Intermediate/Cascade.csv\n\n")

cat("=== Cascade - Prepare Data done ===\n")
